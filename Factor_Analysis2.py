...
def runFactorAnalysis():
    ...
    eigenvalues = np.real(eigenvalues)  # На всякий случай убираем малые комплексные части
    ...
    # Матрица факторных нагрузок: loading = sqrt(lambda) * eigenvector
    loadings = eigenvectors[:, :optimal_factors] * np.sqrt(eigenvalues[:optimal_factors])
    ...
    # Общности (communalities) — сумма квадратов нагрузок по строкам
    ...
    explained_variance = eigenvalues[:optimal_factors]
    proportion_variance = explained_variance / np.sum(eigenvalues)
    cumulative_variance = np.cumsum(proportion_variance)

    variance_df = pd.DataFrame({
        'SS Loadings': explained_variance,
        'Доля объясненной дисперсии': proportion_variance,
        'Накопительная дисперсия': cumulative_variance
    }, index=[f'Factor_{i + 1}' for i in range(optimal_factors)])

    print(f"\nОбъясненная дисперсия:")
    print(variance_df.round(3))

    # Факторные оценки: умножаем стандартизированные данные на матрицу весов (loadings / lambda)
    # Стандартизация данных
    df_std = (df - df.mean()) / df.std()

    # Веса для факторных оценок: loading / lambda
    weights = loadings / eigenvalues[:optimal_factors]
    factor_scores = np.dot(df_std.values, weights)
